...
for(kk in 1:3){
  ...
  print(paste(nrow(distance_matrix), ncol(distance_matrix), "------distance matrix Successfull 4002 X 3828", collapse = " "))
  ...
  #write.csv(inter_distance, "all_interaction_distance_data.csv", row.names = F)
  #write.csv(inverse_inter_distance, "all_inverse_interaction_distance_data.csv", row.names = F)
  
  print(paste("end of", kk, "at", Sys.time(), sep=" "))
